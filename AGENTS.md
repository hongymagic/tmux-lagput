...
- `tests/fixtures*/` contains fake platform commands used by the tests.

## Development

- Preserve compatibility with macOS Bash 3 and current Linux Bash.
- Keep `gum`, `setsid`, and systemd optional; the plain Bash and `nohup`
  fallbacks must continue to work.
- Capture `#{pane_id}` when the binding fires. Never retarget a job based on the
  focused pane at execution time.
